...
class BertQA(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.tag_loss = nn.BCEWithLogitsLoss(pos_weight=torch.tensor(hparams.pos_weight_tag, dtype=torch.float), reduction='sum')
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, pos_tag):
        ...
        return tag_logit, val_start, val_end
    ...
    def _calculate_tag_loss(self, y_hat, y):
        ...
        # loss = self.tag_loss(y_hat.squeeze(1), y[:,0])
        return loss
    ...
    def training_step(self, batch, batch_nb):
        ...
        loss_val = self._calculate_val_loss(logit_start, logit_end, batch["start_idx"], batch["end_idx"], token_type_ids, attention_mask)
...
hparams = Namespace(**{
    'train_dataset_path': "./dataset/train_max100_1.pkl",
    'valid_dataset_path': "./dataset/dev_max100_1.pkl",
    'batch_size': 4,
...
def main(args):
    ...
    trainer = pl.Trainer(gpus=[0], max_epochs=32, gradient_clip_val=1, checkpoint_callback=True, early_stop_callback=True)
