...
class BertQA(pl.LightningModule):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask):
        ...
        # for val
        val_start = self.find_start(last_hidden_state[:,1:101])
        val_end = self.find_end(last_hidden_state[:,1:101])

        return has_tag_logit.squeeze(1), tag_logit, val_start, val_end

    def _unpack_batch(self, batch):
        return batch['input_ids'], batch['token_type_ids'], batch['attention_mask'], batch["tag_n"].float()
    ...
    def training_step(self, batch, batch_nb):
        input_ids, token_type_ids, attention_mask, tag_n = self._unpack_batch(batch)
        has_tag_logit, logit_tag, val_start, val_end = self.forward(input_ids, token_type_ids, attention_mask)
